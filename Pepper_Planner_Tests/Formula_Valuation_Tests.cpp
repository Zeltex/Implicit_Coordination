...
namespace PepperPlannerTests
{
	...
	public:
		...
		TEST_METHOD(Top_Valuation) {
			Formula f;
			f.f_top();
			Assert::IsTrue(f.valuate(std::vector<std::string>{}));
		}

		TEST_METHOD(Bot_Valuation) {
			Formula f;
			f.f_bot();
			Assert::IsFalse(f.valuate(std::vector<std::string>{}));
		}
		...
		TEST_METHOD(And_Valuation_Prop_Exists) {
			...
			Assert::IsTrue(f.valuate(std::vector<std::string>{ "first(a)" , "second(a)" }));
		}
		...
		TEST_METHOD(Big_Formula4) {
			...
			auto component2 = f.f_not({ f.f_and({ f.f_prop( "fourth(a)"), f.f_prop( "fifth(a)"), f.f_prop( "sixth(a)") }) });
			f.f_and({ component1, component2 });
			Assert::IsTrue(f.valuate(std::vector<std::string>{ "second(a)", "third(a)" }));
		}
