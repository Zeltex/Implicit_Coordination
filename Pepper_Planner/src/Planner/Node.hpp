...
namespace del {
	...
	public:
		Node() = delete;
		Node(State state, Node_Id id, Node* parent, const Action* action_from_parent, bool root);
		...
		const std::vector<std::pair<Node*, const Action*>>&	get_parents() const;
	...
	private:
		...
		State state;
		Node_Id id;
		std::vector<std::pair<Node*, const Action*>> parents;
		bool root;
		bool dead;
		bool solved;
