...
namespace del {
	...
	public:
		...
		void partition_into_relations_blocks_contraction(const World_Relations& relations, size_t agents_size, size_t worlds_size);
		...
		std::string convert_propositions_to_string(const std::vector<Proposition>& propositions) const;
	private:
		State state1;
		State state2;
